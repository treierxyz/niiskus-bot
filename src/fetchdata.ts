...
const sqlQuery: string = 'SELECT begin_time AS \"time\", temperature_mean, relhum_mean, baro_mean, sqrt(power(wind_mean_x, 2) + power(wind_mean_x, 2)) AS \"wind_mean\", ((degrees(atan2(wind_mean_x, wind_mean_y)) + 360)::numeric % 360) AS \"wind_dir\", irradiance_mean, (SELECT sum(precip_sum) AS \"percip\" FROM fhilm_primary WHERE begin_time between now() - interval \'1h\' and now()) AS \"percip_sum\" FROM fhilm_primary WHERE $__timeFilter(begin_time) ORDER BY 1 desc limit 1'
